...
**Single Zustand store = the entire app's domain state.** [src/store/useStore.ts](src/store/useStore.ts) holds the data slices `tags`, `tasks`, `hours`, `weekSchedules`, `roadmapPhases`, `importantDates`, `completions` (task-completion log for the progress chart) and `dashboardWidgets` (the configurable dashboard layout) — the `LifeOSData` type in [src/types/index.ts](src/types/index.ts); calendar events use `date: 'yyyy-MM-dd'` strings, compared lexicographically — plus `scheduleWeekKey` (session-only, the week shown in the Horario) and all mutation actions. `Task` carries `urgency`/`importance` (`Priority`); moving a task in/out of `done` (`moveTask`/`updateTask`) appends/removes a `CompletionEvent` in `completions`. Adding a slice is retrocompatible: `hydrate` defaults missing slices (`completions ?? []`, `dashboardWidgets` → `DEFAULT_WIDGETS`). **The store no longer uses the `persist` middleware or seed data** — it starts empty (`createEmptyData()`) and is hydrated per-session via `hydrate(data)`. Consequences:
...
- **Theming (Liquid Glass, light + dark)**: `next-themes` `ThemeProvider` (`attribute="class"`, `defaultTheme="dark"`) in [src/main.tsx](src/main.tsx) toggles `.dark` on `<html>`; a `ThemeToggle` (sun/moon) lives in the `AppShell` top bar. `index.html` keeps `class="dark"` + an inline dark `background-color` as the pre-hydration default (avoids flash). Both `:root` (light) and `.dark` tokens live in [src/index.css](src/index.css) with a **blue/violet accent** and **translucent surfaces**; a fixed `body::before` paints the **aurora** background, and `@layer base` rules add `backdrop-filter` glass to `aside`, `header` and the `[data-slot]` surfaces (card/dialog/popover/select). The app root and `main` are transparent so the aurora shows through. The 9-color `COLOR_PALETTE` is unchanged. Note: `backdrop-filter` is GPU-heavy — glass is applied to large surfaces only, not per schedule cell.
